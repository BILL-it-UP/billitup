...
router.post("/", requireRole("owner", "admin"), (req, res) => {
  const { name, phone, email, billing_address, shipping_address, gstin, state } = req.body;
  if (!name) return res.status(400).json({ error: "name is required" });
  const result = db
    .prepare(
      `INSERT INTO customers (business_id, name, phone, email, billing_address, shipping_address, gstin, state)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?)`
    )
    .run(req.auth.businessId, name, phone, email, billing_address, shipping_address, gstin, state || null);
  res.status(201).json({ id: result.lastInsertRowid, name, phone, email, billing_address, shipping_address, gstin, state });
});
...
router.put("/:id", requireRole("owner", "admin"), (req, res) => {
  ...
  ).run(name, phone, email, billing_address, shipping_address, gstin, state, req.params.id, req.auth.businessId);
